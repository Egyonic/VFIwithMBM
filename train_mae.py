...
def train(model, local_rank, args):
    ...
    step = 0
    nr_eval = 0
    dataset = VimeoDataset('train', small_dataset=args.small_dataset)
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        for i, data in enumerate(train_data):
            ...
            if step % 1000 == 1 and local_rank == 0:
                ...
                for i in range(3):
                    ...
                    writer.add_image(str(i) + '/reconstruct', reconstructed_img[i], step, dataformats='HWC')
                writer.flush()
            if local_rank == 0:
        ...
        if epoch % 5 == 0:
            model.save_model(log_path, epoch, local_rank)    
        dist.barrier()
...
def evaluate(model, val_data, nr_eval, local_rank, writer_val):
    ...
    for i, data in enumerate(val_data):
        ...
        with torch.no_grad():
            ...
            rec_img = info['imgs_reconstructed']
        loss_l1_list.append(info['loss_l1'].cpu().numpy())
        loss_tea_list.append(info['loss_tea'].cpu().numpy())
        loss_distill_list.append(info['loss_distill'].cpu().numpy())
        loss_rec_list.append(info['loss_reconstruct'])
        for j in range(gt.shape[0]):
            psnr = -10 * math.log10(torch.mean((gt[j] - pred[j]) * (gt[j] - pred[j])).cpu().data)
            psnr_list.append(psnr)
            psnr = -10 * math.log10(torch.mean((merged_img[j] - gt[j]) * (merged_img[j] - gt[j])).cpu().data)
            psnr_list_teacher.append(psnr)
            psnr = -10 * math.log10(torch.mean((gt[j] - rec_img[j]) * (gt[j] - rec_img[j])).cpu().data)
            psnr_list_rec.append(psnr)
        gt = (gt.permute(0, 2, 3, 1).cpu().numpy() * 255).astype('uint8')
        pred = (pred.permute(0, 2, 3, 1).cpu().numpy() * 255).astype('uint8')
        merged_img = (merged_img.permute(0, 2, 3, 1).cpu().numpy() * 255).astype('uint8')
        rec_img = (rec_img.permute(0, 2, 3, 1).detach().cpu().numpy() * 255).astype('uint8')
        flow0 = info['flow'].permute(0, 2, 3, 1).cpu().numpy()
        flow1 = info['flow_tea'].permute(0, 2, 3, 1).cpu().numpy()
        if i == 0 and local_rank == 0:
            for j in range(3):
                imgs = np.concatenate((merged_img[j], pred[j], gt[j], rec_img[j]), 1)[:, :, ::-1]
    ...
    writer_val.add_scalar('psnr_reconstruct', np.array(psnr_list_rec).mean(), nr_eval)
    if np.array(psnr_list_rec).mean() > max_psnr:
...
if __name__ == "__main__":    
    ...
    parser.add_argument('--small_dataset', default=False, type=bool, help='use small vimeo90k ( 1/8 data) ')
    parser.add_argument('--weight_path', default=None, type=str, help='pretrained weight path, default in train_log')
    parser.add_argument('--model_name', default='IFNet', type=str, help='pretrained weight path, default in train_log')
    ...
    model = Model(args.local_rank, model_name=args.model_name)
    # 为每个模型的每次训练设置权重保存目录
    t_s = time.strftime('%Y-%m-%d_%H_%M_%S', time.localtime(time.time()))
    log_path = os.path.join(log_path, args.model_name, t_s)
    os.mkdir(log_path)

    if args.weight_path is not None:
        model.load_model(args.weight_path)
        print(f'succesfully load pretrained model in {args.weight_path}')
        print(f'train from epoch {args.start_epoch}')
