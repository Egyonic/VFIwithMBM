...
from model.lomar.vision_transformer_irpe import PatchEmbed, Block

from model.lomar.util.pos_embed import get_2d_sincos_pos_embed
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def generate_window_patches(self, x, left, top, window_size, mask_ratio):
        ...
        column = torch.arange(0, window_size * W, W, device=x.device).unsqueeze(0).expand(window_number,
                                                                                          window_size) + top * W

        in_window_mask_number = int(window_size * window_size * mask_ratio)

        assert in_window_mask_number >= 1
        in_window_patches = row.unsqueeze(1).expand(window_number, window_size, window_size) + column.unsqueeze(
            -1).expand(left.shape[0], window_size, window_size)
    ...
    def forward_encoder(self, x, window_size, num_window, mask_ratio):
        ...
        x, ids_restore, mask_indices = self.generate_window_patches(x, rand_left_locations, rand_top_locations,
