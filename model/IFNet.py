...
from model.refine_tf import Restormer, Restormer_BRA, Restormer_EffcientVit
...
# 使用深度可分离卷积，减少计算量和网络复杂度
class IFBlock_bf_L_Light(nn.Module):
    def __init__(self, in_planes, c=64, tf_dim=64, n_win=7, n_block=4, kv_downsample_mode='identity', topk=4,
                 mlp_ratio=2, num_heads=4, kv_per_win=2):
        super(IFBlock_bf_L_Light, self).__init__()
        self.conv0 = nn.Sequential(
            DSConv(in_planes, c // 2, 3, 1, 1),
            DSConv(c // 2, c, 3, 2, 1),
        )
        self.tf_conv = nn.Sequential(
            DSConv(c, tf_dim, 3, 2, 1),
        )
        self.tf_conv_revert = nn.Sequential(
            deconv(tf_dim, c, 3, 2, 1),
        )
    ...
    def forward(self, x, flow, scale):
        ...
        y = self.tf_block(y)
        y = self.tf_conv_revert(y)
        tmp = self.lastconv(torch.cat((x, y), dim=1))
        tmp = F.interpolate(tmp, scale_factor=scale, mode="bilinear", align_corners=False)
        flow = tmp[:, :4] * scale
        mask = tmp[:, 4:5]
        return flow, mask
...
# 使用可变形卷积，提高精度
class IFBlock_bf_dcn(nn.Module):
    def __init__(self, in_planes, c=64, tf_dim=64, n_win=14, n_block=4, kv_downsample_mode='identity', topk=4,
                 mlp_ratio=3, num_heads=4, kv_per_win=2):
        super(IFBlock_bf_dcn, self).__init__()
        self.conv0 = nn.Sequential(
            DCNconv(in_planes, c // 2, 3, 2, 1),
            DCNconv(c // 2, c, 3, 2, 1),
        )
        self.tf_conv = nn.Sequential(
            conv(c, tf_dim, 3, 1, 1),
        ...
        self.lastconv = nn.ConvTranspose2d(c * 2, 5, 4, 2, 1)
        biformer_blocks = []
        for i in range(n_block):
            biformer_blocks.append(BiformerBlock(
                dim=tf_dim, n_win=n_win, num_heads=num_heads, kv_downsample_mode=kv_downsample_mode,
...
class IFNet_bf_resnet_cbam_M(nn.Module):
    ...
    def visualize(self, x, scale=[4, 2, 1], timestep=0.5):
        ...
        return flow_list, mask_list[
            2], merged, flow_teacher, merged_teacher, tmp, before_res, res, mask_guide, warped_img0, warped_img1, c0, c0_x


class IFNet_bf_light_resnet_cbam_M(nn.Module):
    def __init__(self):
        super(IFNet_bf_light_resnet_cbam_M, self).__init__()
        self.block0 = IFBlock_bf_L_Light(6, c=360, tf_dim=64, n_win=7, n_block=4, topk=4, num_heads=4,
                                         mlp_ratio=2, kv_downsample_mode='ada_maxpool', kv_per_win=1)
        self.block1 = IFBlock_bf_L_Light(13 + 4, c=220, tf_dim=64, n_win=7, n_block=6, topk=4, num_heads=4,
                                         mlp_ratio=2, kv_downsample_mode='ada_maxpool', kv_per_win=1)
        self.block2 = IFBlock_bf_L_Light(13 + 4, c=120, tf_dim=128, n_win=14, n_block=6, topk=4, num_heads=8,
                                         mlp_ratio=2, kv_downsample_mode='ada_maxpool', kv_per_win=2)
        self.block_tea = IFBlock_bf_L_Light(16 + 4, c=120, tf_dim=128, n_win=7, n_block=4, topk=4, num_heads=8,
                                            mlp_ratio=2, kv_downsample_mode='ada_maxpool', kv_per_win=1)
        self.contextnet = resnet50_feature_L()
        self.unet = UnetCBAM_M_L()
...
class IFNet_bf_DCN_resnet_cbam_M(nn.Module):
    def __init__(self):
        super(IFNet_bf_DCN_resnet_cbam_M, self).__init__()
        self.block0 = IFBlock_bf_dcn(6, c=240, tf_dim=192, n_win=7, n_block=4, topk=4, num_heads=4,
                                         mlp_ratio=3, kv_downsample_mode='ada_maxpool', kv_per_win=1)
        self.block1 = IFBlock_bf_dcn(13 + 4, c=150, tf_dim=128, n_win=7, n_block=6, topk=6, num_heads=4,
                                         mlp_ratio=3, kv_downsample_mode='ada_maxpool', kv_per_win=1)
        self.block2 = IFBlock_bf_dcn(13 + 4, c=90, tf_dim=96, n_win=14, n_block=6, topk=8, num_heads=8,
                                         mlp_ratio=3, kv_downsample_mode='ada_maxpool', kv_per_win=2)
        self.block_tea = IFBlock_bf_dcn(16 + 4, c=90, tf_dim=96, n_win=7, n_block=4, topk=2, num_heads=8,
                                            mlp_ratio=3, kv_downsample_mode='ada_maxpool', kv_per_win=1)
        self.contextnet = resnet50_feature()
        self.unet = UnetCBAM_M()
...
class IFNet_bf_resnet_cbam_HM_Res_L_v2(nn.Module):
    def __init__(self):
        ...
        self.block_tea = IFBlock_bf_H_L_v2(16 + 4, c=135, tf_dim=64, n_win=14, n_block=2,
...
class IFNet_bf_resnet_RF_M(nn.Module):
    def __init__(self):
        ...
        self.contextnet = resnet50_feature_L()
        self.unet = Restormer(inp_channels=50, out_channels=3, dim=32, num_blocks=[2, 4, 4, 2], num_refinement_blocks=2)
...
class IFNet_bf_resnet_RF_M_L(nn.Module):
    def __init__(self):
        super(IFNet_bf_resnet_RF_M_L, self).__init__()
        self.block0 = IFBlock_bf_L(6, c=240, tf_dim=192, n_win=7, n_block=4, kv_downsample_mode='ada_avgpool',
                                   topk=4, mlp_ratio=2, kv_per_win=1)
        self.block1 = IFBlock_bf_L(13 + 4, c=150, tf_dim=128, n_win=7, n_block=6, kv_downsample_mode='ada_avgpool',
                                   topk=4, mlp_ratio=2, kv_per_win=1)
        self.block2 = IFBlock_bf_L(13 + 4, c=90, tf_dim=64, n_win=14, n_block=6, kv_downsample_mode='ada_avgpool',
                                   topk=6, mlp_ratio=2, kv_per_win=1)
        self.block_tea = IFBlock_bf_L(16 + 4, c=90, tf_dim=64, n_win=14, n_block=2, kv_downsample_mode='ada_avgpool',
                                      topk=4, mlp_ratio=2, kv_per_win=1)
        self.contextnet = resnet50_feature_L()
        self.unet = Restormer(inp_channels=50, out_channels=3, dim=48, num_blocks=[2, 4, 4, 2], num_refinement_blocks=2)
...
if __name__ == "__main__":
    ...
    flownet = IFNet_bf_DCN_resnet_cbam_M()
