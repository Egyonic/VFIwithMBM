...
from model.mae.pos_embed import get_2d_sincos_pos_embed
import os
import sys
import time
...
def get_rec_patches(pred_imgs, mask, window_size=16, threshold=1):
    ...
    num_patch = int((H / window_size) * (W / window_size))
    ...
    window_region = F.interpolate(window_mark, scale_factor=window_size, mode='nearest')
...
class MaskedAutoencoderViTSpecified(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone
    """
    ...
    def specified_masking(self, x, patch_mask):
        """
        Perform specified masking using patch_mask tensor.
        x: [N, L, D], sequence
        patch_mask: [N, L], binary mask where 1 indicates patches to be masked
        """
        N, L, D = x.shape  # batch, length, dim

        # keep the first subset using inverted patch_mask
        ids_keep = ~patch_mask.bool()  # Take the inverse of patch_mask
        x_masked = torch.masked_select(x, ids_keep.unsqueeze(-1)).reshape(N, -1, D)

        return x_masked

    def forward_encoder(self, x, patch_mask):
        # embed patches
        x = self.patch_embed(x.clone())

        # add pos embed w/o cls token
        x = x + self.pos_embed[:, 1:, :]

        # masking: length -> length * mask_ratio
        x = self.specified_masking(x, patch_mask)
    ...
    def forward_decoder(self, x, patch_mask):
        # embed tokens
        x = self.decoder_embed(x)

        # append mask tokens to sequence
        mask_tokens = self.mask_token.repeat(x.shape[0], patch_mask.shape[1] + 1 - x.shape[1], 1)
        x_ = torch.cat([x[:, 1:, :], mask_tokens], dim=1)  # no cls token
        x_ = torch.gather(x_, dim=1,
                          index=(patch_mask.to(torch.int64)).unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle
        x = torch.cat([x[:, :1, :], x_], dim=1)  # append cls token
...
def mae_vit_spe_base_patch16_dec512d8b(**kwargs):
...
def mae_vit_spe_base_patch8_dec512d8b(**kwargs):
    model = MaskedAutoencoderViTSpecified(
        img_size=56, patch_size=8, embed_dim=768, depth=10, num_heads=12,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
...
def mae_vit_spe_base_patch8_tiny(**kwargs):
    model = MaskedAutoencoderViTSpecified(
        img_size=56, patch_size=8, embed_dim=128, depth=4, num_heads=4,
...
if __name__ == '__main__':
    model = mae_vit_spe_base_patch16_dec512d8b()
    ...
    mask_name = 'rec_patchs_ws_16.png'
    I0 = cv2.imread(os.path.join(img_root, img_name))
    mask_patch = cv2.imread(os.path.join(mask_root, mask_name))
    I0 = (torch.tensor(I0.transpose(2, 0, 1)).to(device) / 255.).unsqueeze(0)
    mask_patch = (torch.tensor(mask_patch.transpose(2, 0, 1)).to(device) / 255.).unsqueeze(0)
    mask_patch = (mask_patch[:, 0, :, :]).unsqueeze(0)
    I0 = I0[:, :, :, :224]
    mask_patch = mask_patch[:, :, :, :224]
    window_mark, window_region = get_rec_patches(I0, mask_patch)

    model(I0, window_mark)
    print('ok')
